...
arithmetic: when a parser gets a figure wrong, does the error break a sum the
document asserts about itself? Ground truth that checks itself is worth
...
| fdic_income | 40 | 97.5% | 97.5% | 47.5% | **92.5%** | 85.0% | 85.0% | 85.0% |
| fdic_earnings | 207 | 99.0% | 98.6% | 93.2% | **94.7%** | 83.6% | 66.2% | 84.5% |
| census29_wages | 44 | 93.2% | 88.6% | 38.6% | **0.0%** | 15.9% | 34.1% | 9.1% |
| census29_sales | 32 | 96.9% | 100.0% | 81.2% | **0.0%** | 25.0% | 68.8% | 34.4% |
| **corpus** | **425** | **98.4%** | **97.9%** | **79.8%** | **78.1%** | **74.6%** | **71.5%** | **74.6%** |
| seconds/page | | 64.1 | 57.4 | 122.5 | **0.0** | 6.6 | 18.4 | 1.3 |

\* born-digital. Every other page is a photograph of paper.

- **One local tool of six loses to doing nothing, and two more only draw with
...
- **Cost.** $1.12 for the whole corpus, both hosted models: Claude
  ...
  buys the difference between 9% and 93%. Both of those are over all 76
  labeled values on the 1929 pages, of which 43 are handwritten and 33 are
  printed form text; handwriting alone, Tesseract recovers 1 of 43.
...
| A figure in prose parentheses is not a negative either -- the currency sign, not the sentence position, is what separates a stated amount from a loss | `tests/test_grader.py::test_a_figure_in_prose_parentheses_is_not_a_negative_either` |
...
| And the thousands rule is off unless a page declares it, because plain text cannot tell `132 434 809` from three adjacent cells | `tests/test_grader.py::test_the_thousands_rule_is_off_unless_the_page_asks_for_it`, `::test_no_page_in_this_corpus_asks_for_the_thousands_rule` |
...
| No rule fails identically for every parser -- seven independent tools do not agree on a mistake, so when they do the checker made it | `tests/test_reconcile.py::test_no_rule_fails_identically_for_every_parser` |
...
their output is committed as evidence rather than regenerated, and the
...
The first corpus was thrown away and this one was specified before it was built,
from four properties; these being what had actually separated the tools rather
than what a corpus is usually assumed to need. All four are in the shipped six
pages, and each one earns its place by a result that would be missing without
it.
...
1. **Handwriting on a poor scan.** `census29_wages` and `census29_sales`, two
   ...
   15.9%, Tesseract 9.1%, and the control 0.0%, because there is no text layer
...
4. **A dense multi-column table.** `fdic_earnings`, Table 114, 33 rows by 11
   ...
   widest: GPT 99.0% down to Docling 66.2%.
...
The obstacle was never copyright; it was privacy. Handwriting that is easy
...
released by the National Archives with neither an access nor a use restriction,
so the corpus gets genuine handwriting without publishing anyone's private
information and without staging the difficulty.
...
because some parser's output format was being scored as damage: HTML tags, HTML
entities, Markdown escapes, diacritics, digit regrouping. If you add a seventh
parser, assume the first run measures its serializer rather than its accuracy,
and read its raw output next to the verdict before believing either.
...
documents it did not get to design, and the two choices of corpus are the trade
being made. A generated page can be made to contain exactly the error you want
to study; a 1956 scan contains the errors it contains, and four of the labels
here were wrong until the page's own arithmetic caught them.
[llm-eval-gate](https://github.com/jkelly-dev1/llm-eval-gate) shares this one's
central worry from the other end. It measures its own judges before it trusts
their verdicts, and refuses to gate inside their noise floor. The errors here
concentrate in the grader rather than in the parsers, which is the same finding
arrived at from the other end: The instrument is part of the experiment, and the
part nobody tests.
