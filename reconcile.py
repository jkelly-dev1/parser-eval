...
THIS IS THE QUESTION A PAYLOAD-ONLY CHECK CANNOT ANSWER. score.py
splits a parser's misses into ABSENT (a hole, loud) and CORRUPT (a same-shape
...
def num(s: str) -> float:
    ...
    THE TEST IS DELIBERATELY NARROW, and the counter-example is already in the
    corpus. Note 4 of the 1956 income statement ends a sentence with
    "($657,500,000)." -- every parser returns it with the trailing paren
    attached, and it is a positive figure inside prose, not a loss. So the
    parentheses must WRAP the whole token and contain nothing but a number:
    a leading currency sign is allowed, a trailing period is not, and
    "(Note 3)" is not a figure at all.
    """
    t = s.strip()
    ...
    # figure at all, and until this guard existed the code did not agree --
    # so a caller that guards num() with try/except never saw the failure it
    # was guarding against, it saw a plausible wrong value instead.
...
def cell_lookup(truth: dict) -> dict[str, tuple[int, str]]:
    ...
    whichever row was labeled last and checked the wrong cell -- while
    reporting HOLDS. `key` disambiguates those rows for reference purposes
    only; score.py still matches parser output on `anchor`, which is what the
    page actually prints. A table that needs `key` for one row is better off
    giving one to every row: mixing short keys and long label fragments across
    36 sum definitions is how a reference gets pointed at the wrong row by
    hand.
...
def tolerance_of(spec: dict) -> float:
    ...
    total -- they miss by up to 4 in the US column, and 19 of its 36 column
    sums are off by 1 to 4. That is the page being internally consistent in
    dollars and inconsistent in the thousands it prints. Refusing a tolerance
    there does not buy strictness, it just throws away 36 checks.
...
def check_identities(spec_file: Path, truths: dict,
    ...
    Every term must come from the SAME PARSER, which is the point -- this
    grades one tool's coherence across a document set, not agreement between
    tools.
...
def check_sums(truth: dict, fields: dict | None) -> list[dict]:
    ...
    for spec in recon.get("sums", []):
        ...
        # the remainder back to the deduction -- because a - b = c rearranges
        # to c + b = a, which has one term on the right and fits `equals`.
        #
        ...
        # names here contain digits -- fund_closing_1956, note4_face_value --
        # and num() strips everything that is not a digit, so asking "does it
        ...
        if want_key is not None:
            if fields is None:
                ...
                if got is None:
                    ...
                    # being graded is the one thing in this program that is
                    # expected to return garbage, and garbage in one cell of
                    # one document threw away the results for four others.
                    try:
                        want = num(got)
                    except ValueError:
                        status = "INCOMPLETE"
    ...
    for spec in recon.get("row_rules", []):
        ...
        # `equals` -- cell names within the same row -- and asserts that the
        # terms add to the total, which is the same shape as a `sums` entry
        # turned sideways.
        #
        # THE SIDEWAYS FORM IS WHAT A WIDE TABLE NEEDS. Table 114 asserts, on
        ...
        if "of" in spec:
            for i, r in enumerate(rows):
                ...
                # identity is at its most informative because three of the
                # four terms are blank and the fourth must equal the total.
                # The checker reported 62 rules where the page asserts 69 and
                # said nothing about the 7 it dropped, which is the failure
                # this project keeps meeting: a check that quietly declines.
                #
