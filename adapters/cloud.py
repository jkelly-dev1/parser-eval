...
twice, one image, one instruction, one block of text back, and the only thing
a cross-provider comparison can afford to vary is the provider. So the prompt,
the pixels, the resize, the effort level and the output handling are written
once here and neither adapter is allowed its own version.
