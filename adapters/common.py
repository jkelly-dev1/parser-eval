...
the whole point of the exercise is comparing parsers, and a comparison where
each parser was invoked slightly differently measures the invocation. So each
adapter takes the same arguments, reads the same frozen corpus, and writes one
.txt of whatever text the parser produced plus one record with the wall-clock
time and the version of the thing that produced it.
...
def parse_args(parser_name: str) -> argparse.Namespace:
    ...
    # Marker hangs on PO00003 -- the request times out, the server keeps
    # burning CPU, and nothing after it in the job list ever runs. Skipping the
...
def pages(args) -> list[dict]:
    ...
    so every documented command in the status file died on
    `KeyError: '150'` before parsing a single page. A corpus states which
    renderings it has in index.json and that is the answer to the question.
