...
THE SECOND HALF IS THE WHOLE POINT AND IT IS THE PART NOBODY PUBLISHES. A
check that only inspects the extracted payload is blind to a value that is
internally consistent and wrong, and those are precisely the errors that reach
production. A parser leaderboard reports one number, accuracy, and by that
measure a parser with half the error rate and twice the quiet fraction looks
twice as good while being MORE dangerous. So every miss here is classified:
...
with no arithmetic relation to anything -- a SKU, a PO number, a vendor --
would not. The split reported here is the input to that question, not the
answer to it. reconcile.py asks the arithmetic half.
...
def fold(s: str) -> str:
    ...
    # discrimination -- the same argument that makes matching
    # case-insensitive.
    ...
    # money as "\$7,126.11" -- the backslash is Markdown's escape so the
    # dollar sign is not parsed as a math delimiter. Without this the token
    ...
    s = re.sub(r"<[^>]+>", " ", s)
    # SPACE-SEPARATED THOUSANDS ARE ONE NUMBER, NOT THREE TOKENS. The Slovak
    # cash-flow page prints 132 434 809, and splitting on whitespace would
    # turn every figure on it into three unrecognizable fragments -- so every
    # parser would score zero on the page whatever it actually returned.
    #
    # The pattern is deliberately narrow: 1-3 digits followed by one or more
    # groups of EXACTLY three, ending on a word boundary. "05-18 05-19" is two
    # dates and is left alone (05 is a two-digit group); "1000 Walnut" is left
    # alone (1000 is four digits).
    s = join_thousands(s)
...
def lines_of(text: str) -> list[str]:
    out = []
    for raw in fold(text).splitlines():
...
def tokens_of(line: str) -> list[str]:
    ...
    split makes that one token that matches nothing -- the grader then
    reported Kolkata as an error whose closest look-alike was Kolkata, at edit
    distance zero. A field the parser read perfectly must not be scored as a
    miss because of a missing space.
    ...
    it into a negative; an unbalanced paren from prose -- "($657,500,000)." at
    the end of a sentence in note 4 -- is still ordinary punctuation.
    """
    out = []
...
def canon(s: str) -> str:
    ...
    while a parser that returns "29,569" -- the loss with its sign discarded
    -- is still wrong, which is the whole reason the minus is protected above.
    """
...
# number, so "(Note 3)" is not a figure and "($657,500,000)." is a positive
# figure that happens to end a sentence.
ACCOUNTING_NEG_RE = re.compile(r"\(\s*\$?\s*([\d,. ]*\d[\d,. ]*)\s*\)")
...
def concat_match(printed: str, toks: list[str]) -> bool:
    ...
    the Slovak page's "22 167 087" as "22 167087" and "4 070 210" as
    "4070 210" -- same digits, different spaces. Scored token by token those
    are misses, and the reconciliation pass then reads them as figures that
    came back wrong while the row still balances -- which is the signature of
    the quiet error this exercise hunts, produced by spacing alone.
...
def is_multiword(printed: str) -> bool:
    """True when the value must be matched as a substring rather than a token.

    Asked AFTER thousands are joined, so "132 434 809" counts as one token and
    takes the token path. Asking before would send every figure on a page that
    spaces its thousands down the substring path, where a value could be
    credited to a line it merely overlaps.
    """
    return " " in join_thousands(printed).strip()
...
def classify_miss(printed: str, toks: list[str], lines: list[str],
    ...
    `other_texts` is the same set unfolded, used to mask multi-word candidates
    -- see mask_claimed().
    """
    ...
    # error on the page: Tesseract reads the
    # SKU "LN-10NY" as "LN-1ONY" -- a zero recognized as the letter O. That
    # token fails the SKU pattern precisely BECAUSE it is corrupt, so the
    # shape filter discarded it and the field was scored ABSENT. A classifier
    # that can only see corruption which stays inside the schema would report
    # the quiet errors as loud ones, which is the exact inversion of what this
...
def row_blocks(lines: list[str], rows: list[dict]) -> dict[int, list[str]]:
    ...
    for i, row in enumerate(rows):
        ...
        for n, ln in enumerate(lines):
            if n in used:
                continue
            hit = (substr_match(anchor, [ln]) if is_multiword(anchor)
            ...
            score = sum(1 for c in cells
                        if (substr_match(c, window) if is_multiword(c)
...
def grade_page(truth: dict, text: str) -> dict:
    ...
    def grade(name: str, want: str, scope_lines: list[str] | None) -> None:
        ...
        hit = (substr_match(want, sl) if is_multiword(want)
        ...
        # and it is the one a text-only comparison would otherwise hide. The
        ...
        elsewhere = (substr_match(want, lines) if is_multiword(want)
...
def main() -> int:
    ...
    # documents, once re-rendered as scans at two resolutions -- and copying
    # the truth files next to each rendering would be two sets of ground truth
